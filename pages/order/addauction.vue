	...
	<view>
		...
		<view class="container">
			<view class="goods-info mb20">
				...
				<view class="wp100 goods-msg flex-between fdc">
					...
					<view class="flex-between ai-fe">
						<view><text class="fs-20 fc-909">买入价：</text><text class="fs-24 fc-e31">￥</text><text
			...
			<view class="auction-price-box">
				<view class="fs-28 fc-303 lh-40 mb16">转卖售价</view>
				...
				<view class="fs-24 lh-34 fc-939" v-if="lotObj.lotGoodsOrphan===1">
					注：转卖价不能高出买入价{{lotObj.lotGoodsOrphan===1?auctionConfig.auctionMoneyUpLimit:auctionConfig.auctionMoneyUpLimit2}}%，即最高可卖¥{{maxLotOutPrice}}
					</view>
					<view class="fs-24 lh-34 fc-939" v-if="leaveLotCount>0">
					您的物品已经流拍{{leaveLotCount}}次了，流拍3次之后只能留下自用
					</view>
			</view>
			...
			<view class="remark-list flex-between ac">
				<view class="fs-28 fc-303 fw-b">申请挂卖场次时间</view>
		...
		<view class="cu-bar dflex fdc bg-white tabbar border submit-box">
			<view class="wp100 pt12 pb12">
				<label class="dflex jc-fs ac wp100" @tap="changeCheck">
					...
					<view class="fs-22 lh-32 fc-939">我已阅读并同意 <text class="fc-2e8" @click.stop="toAgreement">《转卖委托协议》</text>
					</view>
				</label>
			</view>
			<view class="wp100 flex-between ac pt12 pb12">
				<view class="action">
					...
					<view class="fs-22 lh-32 fc-939 tl" v-show="lotOutPrice>=lotObj.lotInPrice">佣金为转卖溢价部分的{{auctionConfig.auctionUpAddRate}}%</view>
					<view class="fs-22 lh-32 fc-939 tl" v-show="lotOutPrice<lotObj.lotInPrice">佣金为转卖跌价的{{auctionConfig.auctionLowerRate}}%</view>
				</view>
		...
		<view class="cu-modal agreement-modal" :class="{show:showPop}">
			<view class="cu-dialog">
				<view class="cu-bar bg-white justify-end">
					<view class="content fw-b">转卖委托协议</view>
		...
		methods: {
			...
			pubAuctionLot() {
				...
				if(!this.putLotObj.lotOutPrice){
					uni.$toast.showError('请输入转卖售价')
					return 
				}
				if(this.putLotObj.couponType && !this.putLotObj.autionDate){
					uni.$toast.showError('请选择挂卖场次时间')
					return
				}
				...
				uni.$api.pubAuctionLot(this.putLotObj).then(res => {
					uni.$toast.showSuccess('转卖成功！')
					setTimeout(function(){
									uni.navigateTo({
										url:'/pages/user/auctiongoods?lotType=2'
									})
					},400)

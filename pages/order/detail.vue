	...
	<view>
		...
		<view class="container">
			...
			<view class="order-info-box">
				...
				<view class="info-list" v-if="order.orderLotId">
					<view class="fs-24 lh-34 fc-303">物品编号</view>
					<view class="fs-24 lh-34 fc-303">{{order.orderLotId || ''}}</view>
				</view>
				<view class="info-list">
				...
				<template v-if="order.orderType == 11 && order.orderStatus >= 200 && order.orderStatus < 300">
			...
			<view class="order-info-box">
				<view class="fs-28 lh-40 fw-b fc-303">盲盒信息</view>
				<view class="info-list">
					<view class="fs-24 lh-34 fc-303">盲盒编号</view>
					<view class="fs-24 lh-34 fc-303">{{goodsGameBox.goodsBarcode}}</view>
				</view>
				<view class="info-list">
				...
				<!-- <view class="info-list" v-if="order.orderStatus >= 100 && order.orderOpType"> -->
					<!-- 待回收 待发货 -->
					<!-- <view class="fs-24 lh-34 fc-303">订单情况</view> -->
					<!-- <view class="fs-24 lh-34 fc-303" v-if="order.orderOpType == 1">发货</view> -->
					<!-- <view class="fs-24 lh-34 fc-303" v-if="order.orderOpType == 2">违约回收</view> -->
				<!-- </view> -->
	<!-- 			<view class="info-list" v-if="orderAddress">
					<view class="fs-24 lh-34 fc-303">发货数量</view>
					<view class="fs-24 lh-34 fc-303">{{goodsGameBoxItems.length}}</view>
				</view> -->
			</view>
			<view class="order-info-box" v-if="goodsGameBox">
				<view class="goods dflex ai-fs" v-for="item in goodsGameBoxItems">
					<image :src="item.goodsImage" class="goods-image"></image>
					<view class="dflex fdc ai-fs">
						<view class="fs-28 lh-34 fc-303 fw-b mb16">{{item.goodsTitle}}</view>
						<view class="fs-24 lh-28 fc-939 mb12">盒子编号{{item.goodsItemBarcode}}</view>
						<!-- <view class="fs-24 lh-34 fc-303 dflex"><view class="fc-939">市场价：</view> {{item.goodsItemOriginalPrice}} 魔石</view> -->
						<!-- <view class="fs-24 lh-34 fc-303 dflex"><view class="fc-939">回收价：</view> {{item.goodsItemRecyclePrice}}  魔石</view> -->
					</view>
				</view>
			</view>
		</view>
		...
		<view class="cu-bar bg-white tabbar border order-box">
 			<view  v-if="order.orderStatus == 100 && order.orderType == 21">
				<button class="cu-btn line-btn fc-303 round ml24" @click="onRecycle">{{order.orderRecycleMoney}}魔石回收</button>	
				<button class="cu-btn line-btn fc-303 round ml24" @click="showAddress=true">发货</button>	
			</view>
			
			
		 	<view v-if="order.orderStatus == 100 && order.orderType == 11 && order.lotStatus == 1">
				<button class="cu-btn line-btn fc-303 round ml24"  @click="depositAndSell(order.orderLotId)">寄存并转卖</button>
				<button class="cu-btn line-btn fc-303 round ml24"  @click="sendLotPackage(order.orderId)">发货</button>
			</view>
			<view v-if="order.orderStatus == 100 && order.orderType == 11 && order.lotStatus > 1">
				<button class="cu-btn line-btn fc-303 round ml24"  @click="toLotDetail(order.orderLotId)">查看物品详情</button>
			</view>
		...
		<chooseaddress v-if="showAddress" :orderId="orderId" @success="initPage()"  @cancel="showAddress = false"></chooseaddress>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			toLotDetail (orderLotId) {
				uni.navigateTo({
					url:"/pages/user/auctiongoods/detail?lotId="+orderLotId
				})
			},
			...
			initPage () {
				...
				uni.$api.getOrderDetail(params).then(res => {
					...
					this.goodsGameBox = res.data.goodsGameBox || []
					this.goodsGameBoxItems = res.data.goodsGameBoxItems || []

<template>
	<view>
		<cu-custom bgColor="bg-f6" isBack backMethod="backHandle" @back-handle="backHandle">
			...
			<block slot="content">物品管理</block>
		</cu-custom>
		<view class="ordertype flex-between ac" v-if="needDeliveryOrderCounts>0 || deliveryOrderCounts>0">
			<view class="fs-24 lh-24 fc-606">待发货订单{{needDeliveryOrderCounts}}个，待收货订单{{deliveryOrderCounts}}个</view>
			<view class="fs-24 lh-24 fc-2e8" @tap="toOrder">点此查看</view>
		</view>
			...
			<scroll-view scroll-y="true" :style="'height: calc(100vh - '+topBarHeight+'rpx - ' + CustomBar +'px)'" @scrolltolower="loadMore">
				<view class="order-list" v-for="item in lotList" :key="item.lotId">
					<view class="flex-between ac mb32">
						...
						<!-- <view class="fs-24 lh-34 fc-ff4" v-if="item.lotStatus===2">转卖中</view>
						<view class="fs-24 lh-34 fc-ff4"
					...
					<view class="goods-info dflex ai-fs mb16">
						...
						<view class="flex-1">
							<view class="flex-between ai-fs mb16" @tap="toLotDetail(item.lotId)">
								<view class="goods-label fs-0 mt6 mr8" v-if="item.lotGoodsOrphan===1">珍品</view>
					...
					<view class="dflex ac jc-end tr">
						<view class="fs-24 lh-34 fc-303">买入价</view>
					...
					<view class="dflex ac jc-end mt24">
						<view class="small-radio-btn line-btn flex-center"
							v-if="item.lotStatus<3 && (item.lotHangupStatus!==1 && item.lotHangupStatus!==2)"
							@tap="toPubLot(item.lotId)">转卖</view>
						<view class="small-radio-btn line-btn flex-center"
							v-if="item.canPut"
							@tap="toPubLot(item.lotId)">转卖</view>
						<view class="small-radio-btn pp-line-btn flex-center" @tap="toAddress(item.lotOrderId)"
							v-if="(item.lotStatus===1|| (item.lotHangupStatus===3 && item.lotStatus !==3))&& (item.lotHangupStatus!==1 && item.lotHangupStatus!==2)">立即发货</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				lotList: [],
				needDeliveryOrderCounts:0,
				deliveryOrderCounts:0,
				queryObj: {
					lotType: ''
				},
				pageIndex: '',
		...
		methods: {
			...
			getAuctionLots() {
				...
				uni.$api.getAuctionLots(this.queryObj).then(res => {
					...
					this.needDeliveryOrderCounts = res.needDeliveryOrderCounts ||0
					this.deliveryOrderCounts = res.deliveryOrderCounts ||0
					if(this.needDeliveryOrderCounts===0 && this.deliveryOrderCounts===0){

		...
		<view class="container">
			...
			<view class="">
				<view class="dflex ac screen-record">
					<view class="fs-26 lh-90 mr14" :class="{cur:tabbarIndex==0}"  @click="tabbarIndex=0,reset(),onFetchData()">盲盒交易记录 <view class="cur-icon"></view></view>
					<!-- <view class="fs-26 lh-90" :class="{cur:tabbarIndex==1}" @click="tabbarIndex=1,reset(),onFetchData()">未卖出记录<view class="cur-icon"></view></view> -->
				</view>
				<view class="order-list" v-for="item in list.rows">
					<view class="flex-between ac mb32">
						<view class="fs-24 lh-34 fc-303">{{item.orderPayTime}}</view>
				...
				<!-- 	<view class="dflex ac jc-end tr" v-if="tabbarIndex!=0">
					...
					<view class="dflex ac jc-end tr">
						<view class="fs-24 lh-34 fc-303">交易魔石</view>
						<text class="fs-24 fc-303"></text>
						<view class="fs-36 lh-44 fc-303 fw-b">{{item.orderPayMoney}}</view>
					</view>
				</view>
			...
			onFetchData(){
				...
				Promise.resolve().then(()=>{
					// if (this.tabbarIndex == 0){
						return uni.$api.getMhGoodsGameBoxBuyList(this.queryParam)
					// }
					// else {

	...
	<view>
		...
		<view class="flex-between ac pl32 pr32 auction-title-box">
			...
			<view class="flex-center fdc auction-title-info" v-if="(lotObj.lotStatus!==3) && (lotObj.lotHangupStatus===0 || lotObj.lotHangupStatus===3)">
				<view class="fs-36 lh-50 fc-f fw-b mb4">已买入</view>
				<view class="fs-24 lh-34 fc-f"><text class="op75">您可以选择转卖或自用</text></view>
			</view>
			<view class="flex-center fdc auction-title-info" v-if="auctionLot.auctionLotHangupStatus===4">
				<view class="fs-36 lh-50 fc-f fw-b mb4">已卖出</view>
				<view class="fs-24 lh-34 fc-f"><text class="op75">您的物品已卖出</text></view>
			</view>
			<view class="flex-center fdc auction-title-info" v-if="lotObj.lotHangupStatus===1">
				<view class="fs-36 lh-50 fc-f fw-b mb4">转卖中 - 申请挂卖</view>
				<view class="fs-24 lh-34 fc-f"><text class="op75">等待转卖人员的审核</text></view>
			</view>
			<view class="flex-center fdc auction-title-info" v-if="lotObj.lotStatus===2&&lotObj.lotHangupStatus===2">
				<view class="fs-36 lh-50 fc-f fw-b mb4">转卖中</view>
		...
		<view class="container">
				...
				<view class="goods-info">
					...
					<view class="wp100 dflex fdc">
						<view class="dflex fdc jc-fs goods-msg">
							<view class="dflex ai-fs mb12">
								<view class="goods-label fs-0 mt6 mr8" v-if="lotObj.lotGoodsOrphan===1">珍品</view>
								<view class="goods-name">{{lotObj.lotName}}</view>
							</view>
							<view class="fs-20 lh-28 fc-939"></view>
						</view>
						<view>
							<text class="fs-22 fc-606">买入价：</text>
			...
			<view class="order-info-box" v-for="item in lotLogs" :key="item.auctionLotAutoId" >
				<view class="fs-28 lh-40 fw-b fc-303 mb32" v-once>挂卖记录</view>
				...
				<view class="info-list">
					<view class="fs-24 lh-34 fc-303">挂卖价</view>
			...
			<view class="order-info-box">
				<view class="fs-28 lh-40 fw-b fc-303">物品信息</view>
				...
				<view class="info-list">
					<view class="fs-24 lh-34 fc-303 ">买入时间</view>
			...
			<view class="order-info-box" v-if="lotObj.lotHangupStatus===1">
				<view class="fs-28 lh-40 fw-b fc-303">申请转卖信息</view>
				<view class="info-list" >
					<view class="fs-24 lh-34 fc-303 ">申请挂卖场次时间</view>
		...
		<view class="cu-bar bg-white tabbar border order-box">
			...
			<button class="cu-btn line-btn fc-303 round mr24" v-if="(lotObj.lotStatus===1|| (lotObj.lotHangupStatus===3 && lotObj.lotStatus !==3)) && (lotObj.lotHangupStatus!==1 && lotObj.lotHangupStatus!==2)" @tap="toAddress">立即发货</button>
			...
			<button class="cu-btn bg-pp fc-f round" v-if="lotObj.lotStatus===1&& (lotObj.lotHangupStatus!==1 && lotObj.lotHangupStatus!==2 && lotObj.lotHangupStatus!==3)" @tap="toPubLot">转卖</button>
			<button class="cu-btn bg-pp fc-f round" v-if="lotObj.canPut" @tap="toPubLot">转卖</button>
		</view>

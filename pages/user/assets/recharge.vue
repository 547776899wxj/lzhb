<template>
	<view>
		...
		<view class="container">
			<view class="fs-32 lh-32 fc-3 fw-b mb42">剩余魔石 {{money}}</view>
			<view class="fs-28 lh-28 fc-303 mb20">充值金额</view>
			<view class="form-group mb20">
				<input placeholder="请输入充值金额" name="input"
				 type="digit" maxlength="10"
				  placeholder-class="fc-c4cc" v-model="model.money" @input="onMoneyInput">
			</view>
			<view class="fs-28 lh-28 fc-393 mb50">充值金额数量比例1:1</view>
			...
			<label class="dflex jc-fs ac wp100 mb80" @click="model.payCheck = !model.payCheck">
				<checkbox class="hh-radio fs-0 mr16 round" :class="model.payCheck?'checked':''"
					:checked="model.payCheck"></checkbox>
				...
				uni.$api.rechargeScore(this.model).then(res => {
					...
					if (istest) { //测试支付
						...
						if (self.model.channelType === 4) {
							...
							plus.runtime.openURL('alipays://platformapi/startapp?appId=20000067&url=' +
								encodeURIComponent(res.orderInfo.qrCode));
						...
						if (self.model.channelType === 5) {
							...
							if (uni.getSystemInfoSync().platform === 'ios') {
								plus.runtime.openURL('https://ds.alipay.com/?from=mobilecodec&scheme=' +
									encodeURIComponent(
										'alipays://platformapi/startapp?saId=10000007&clientVersion=3.7.0.0718&qrcode=' +
										encodeURIComponent(res.orderInfo + '?s=web-other')));
							} else {
								plus.runtime.openURL(
									'alipays://platformapi/startapp?saId=10000007&clientVersion=3.7.0.0718&qrcode=' +
									encodeURIComponent(res.orderInfo));
							}
							clearInterval(self.timers || 0)
							self.timers = setInterval(self.checkOrderStatus, 2000)
							setTimeout(() => self.showConfirm = true, 1000)

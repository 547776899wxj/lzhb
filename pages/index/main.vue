<template>
	<view id="containerId">
		<view class="bg-white dflex ac pl48" :style="style"><view class="fs-36 lh-36 fw-b fc-464">首页</view></view>
		<view class="siwper-box mb20" v-if="indexAdvList.length > 0">
			...
			<swiper :indicator-dots="false" :autoplay="true" :interval="3000" :duration="1000" :circular="true" @change="swiperChange">
				<swiper-item v-for="item in indexAdvList" @click="navTo(item)">
					<view class="swiper-item"><image :src="item.advUrl" class="banner"></image></view>
				</swiper-item>
			</swiper>
			<view class="swiper-dot"><view :class="swiperKey == index ? 'active' : ''" v-for="(item, index) in indexAdvList" :key="index"></view></view>
		</view>
		<view class="content">
			<view class="special-area mb20">
				...
				<view class="new-people" @click.stop="toNewPeople()">
					...
					<view class="dflex fdc ai-fe special-text"><view class="fs-26 lh-26 fc-464 fw-b">新人须知</view></view>
				</view>
			</view>
	...
	methods: {
		...
		navTo(item) {
			...
			if (item.advNavigationUrl) {
				if (item.advNavigationUrl == '/pages/user/index' || item.advNavigationUrl == '/pages/cabinet/index' || item.advNavigationUrl == '/pages/collection/index') {

<template>
	<view class="template">
		...
		<view class="classify-box mb20">
			<view class="classify flex-center fdc" @click.stop="toBlindbox()">
				<image src="../../static/img/icon/classify1.png" class="classify-img"></image>
				<view class="fs-24 lh-24 fc-6">娱乐盲盒</view>
			</view>
			<view class="classify flex-center fdc" @click.stop="toRecharge">
				<image src="../../static/img/icon/classify2.png" class="classify-img"></image>
				<view class="fs-24 lh-24 fc-6">魔石充值</view>
			</view>
			<view class="classify flex-center fdc" @click.stop="toHelp(3)">
				<image src="../../static/img/icon/classify3.png" class="classify-img"></image>
				<view class="fs-24 lh-24 fc-6">发货流程</view>
			</view>
			<view class="classify flex-center fdc" @click.stop="toHelp(4)">
				<image src="../../static/img/icon/classify4.png" class="classify-img"></image>
				<view class="fs-24 lh-24 fc-6">新手帮助</view>
			</view>
		</view>
		...
		<view class="hot-box mb24">
			<view class="flex-between ac mb28">
				<view class="hot-title fs-32 lh-32 fw-b fc-3">最新开箱</view>
				<view class="fs-24 lh-24 fc-626" @click.stop="toNewOpenBoxList()">更多</view>
			</view>
			<swiper :vertical="true" :autoplay="true" :interval="3000" :duration="1000" :circular="true">
				<swiper-item v-for="newOpenBox in newOpenboxList">
					<view class="swiper-item">
						<view class="dflex">
							<view class="hot-list flex-center fdc" v-for="(item,index) in newOpenBox">
								<image :src="item.goodsImage"></image>
								<view class="fs-24 lh-24 fc-3">￥{{item.goodsItemOriginalPrice}}</view>
							</view>
						</view>
					</view>
				</swiper-item>
		...
		<view class="recommend-box">
			<view class="flex-center mb28">
				...
				<view class="fs-32 lh-32 fc-d52 fw-b ml20 mr20">新品推荐</view>
				<image src="../../static/img/icon/recommend.png" mode="widthFix" class="recommend-icon trans180">
				</image>
			</view>
			<view class="recommend-content flex-between ai-fs">
				<view class="dflex fdc">
					<view class="recommend-list" v-for="(item,index) in leftGoods" :keys="index"
						@click.stop="toBlindboxDetail(item.goodsId)">
						<image :src="item.goodsImage" mode="widthFix" class="recommend-img"></image>
						<view class="recommend-info">
							<view class="fs-28 lh-28 fc-f wp100 text-hidden recommend-price flex-center mb18">
								价值：{{item.minPrice || 0}}元-{{item.maxPrice || 0}}元</view>
							<view class="fs-28 lh-28 fc-3 recommend-name mb30">{{item.goodsTitle}}</view>
							<view class="fs-34 lh-34 fc-fb2 fw-b dflex ai-fe">
								<view class="fs-26 lh-26">￥</view>{{item.goodsPrice}}
							</view>
						</view>
					</view>
				</view>
				<view class="dflex fdc">
					<view class="recommend-list" v-for="(item,index) in rightGoods" :keys="index"
						@click.stop="toBlindboxDetail(item.goodsId)">
						<image :src="item.goodsImage" mode="widthFix" class="recommend-img"></image>
						<view class="recommend-info">
							<view class="fs-28 lh-28 fc-f wp100 text-hidden recommend-price flex-center mb18">
		...
		<view class="cu-modal" :class="showNewUserCoupon?'show':''">
			<view class="novice">
				<image src="/static/img/bg/index-alert2@2x.png" class="wp100 alert-image" style="height: 398px;"
					mode="scaleToFill"></image>
				<view class="coupon-box">
					<image src="/static/img/bg/index-alert-box.png" class="wp100 index-alert-box" style="height: 234px;"
						mode="scaleToFill"></image>
					...
					<view class="coupon-list mb40" v-if="auctionCoupon">
						...
						<view class="coupon-info">
							<view class="coupon-price dflex ai-fe">
								<image src="../../static/img/icon/index-vip-icon@2x.png" class="index-vip-icon" mode="">
								</image>
							</view>
							<view class="coupon-fg-line"></view>
							<view class="coupon-condition">
				...
				<image src="/static/img/icon/index-alert-close@2x.png" class="close-image" mode="" @tap="hideModal">
				</image>
			</view>
		</view>
		<!-- tabbar 占位 -->
	...
	export default {
		...
		methods: {
			...
			initPageData() {
				...
				// this.getCommonPosterList()
				this.getNewOpenBoxList()
				this.getIndexGameboxList()
			},
			getIndexGameboxList() {
				uni.$api.getIndexGameboxList().then(res => {
					var gameboxArr = res.rows || []
					var leftGoods = []
					var rightGoods = []
					if (gameboxArr && gameboxArr.length > 0) {
						for (var i = 0; i < gameboxArr.length; i++) {
							if (i % 2 == 0) {
								leftGoods.push(gameboxArr[i])
							} else {
								rightGoods.push(gameboxArr[i])
							}
						}
					}
			...
			getNewOpenBoxList() {
				...
				uni.$api.getNewOpenBoxList(data).then(res => {
					...
					this.newOpenboxList = group(this.newOpenBox,4)
				})
			...
			getCommonPosterList() {
				uni.$api.getCommonPosterList({
					pageSize: 100
				}).then(res => {
					this.posters = res.rows || []
				})
			},
			toPosterDetail(posterId) {
				uni.navigateTo({
					url: "/pages/info/poster/detail?posterId=" + posterId
				})
			},
			clickToSearchImg() {
				var e = {
			...
			toBlindbox(goodsIpId) {
				console.log("toBlindbox:", goodsIpId)
				if (goodsIpId) {
					uni.switchTab({
						url: "/pages/blindbox/list?goodsIpId=" + goodsIpId
					})
				} else {
					uni.switchTab({
						url: "/pages/blindbox/list"
					})
				}
			},
			toBlindboxDetail(goodsId) {
				uni.navigateTo({
...
<style scoped>
	...
	.template .banner swiper swiper-item {
		width: 100%;
	}
